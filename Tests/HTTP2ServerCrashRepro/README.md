...
3. `HTTP2ClientRequestHandler.run(.forwardResponseEnd)` delivers the response to the caller and clears its reference:
   ...
   self.request = nil
   ```
4. While the client's request body is still streaming, the server issues a `RST_STREAM` frame.
...
7. `HTTP2ClientRequestHandler.run(.failRequest)` force-unwraps `self.request!`:
   ```swift
...
- **Connection Resets**: In high-concurrency environments or when connections are reclaimed, server-side resets (`RST_STREAM`) or connection closures cause `AsyncHTTPClient` to trigger this unhandled nil unwrap rather than throwing a catchable Swift error. This terminates the process instead of allowing retry or backoff policies to recover.

## Why HTTPS / TLS Is Required

`AsyncHTTPClient` does not support cleartext HTTP/2 (`h2c` / prior knowledge). For all `http://` schemes, `AsyncHTTPClient` defaults to HTTP/1.1 (`HTTPConnectionPool+Factory.swift:256`) and does not initialize `HTTP2ClientRequestHandler`. Therefore, negotiating HTTP/2 requires HTTPS with ALPN (`"h2"`), which the reproducer configures using a self-signed certificate with `certificateVerification = .none`.
