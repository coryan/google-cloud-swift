...
@main
struct StorageClientUploadRepro: AsyncParsableCommand, Sendable {
  ...
  @Flag(name: .customLong("resumable"), help: "Use resumable upload instead of simple upload.")
  ...
  func run() async throws {
    ...
    logToStderr(
      "Starting StorageClient repro with \(taskCount) tasks, \(clientCount) clients, \(objectSize) bytes payload, streaming: \(streamBody), resumable: \(resumable)..."
    )
  ...
  private func runWorker(
    ...
    let options = UploadOptions().with {
      if resumable {
        ...
        $0.resumableUploadThreshold = Int.max
      }
    }
